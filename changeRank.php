<?php
	...
	*/
	...
	function updateRank($group,$userId,$rank,$xcsrf='',$cookie,$ranks,$roles,$rankLimit=255) {
		...
		$url = "http://www.roblox.com/groups/api/change-member-rank?groupId=$group&newRoleSetId=".getRoleSet($rank)."&targetUserId=$userId"; // Get rank URL
		$curl = curl_init($url);
		...
		if ($header['Set-Cookie']) { // If the user is logged in it shouldn't set cookies.
			login();
			return updateRank($userId,$rank,$xcsrf,$cookie,$rankLimit);
		} else if (curl_getinfo($curl, CURL_HTTP_CODE) == 403) { // 403 = XCSRF Token Validation Failed
